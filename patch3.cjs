...
if (html.includes('id="chat-messages"')) {
    ...
    // Let's replace the last </main> with the new section + </main>
    const parts = html.split('</main>');
    if (parts.length > 1) {
        // Last part is the footer and everything else after </main>
        const afterMain = parts.pop();
        // The beforeMain is everything before the last </main>
        const beforeMain = parts.join('</main>');
        html = beforeMain + insertionPoint + afterMain;
